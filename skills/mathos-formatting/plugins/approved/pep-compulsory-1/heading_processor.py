...
def get_target_root():
    ...
    default = Path(__file__).parent if '__file__' in dir() else Path.cwd()
    ...
    if user_input:
        ...
        if target.is_dir():
            ...
            print(f"Directory '{user_input}' not found, using default.")
    return default
...
def replace_in_file(path):
    ...
    # Extract TOC entries from protected text (TOC itself is not protected)
    ...
    # Apply heading normalization based on TOC
...
def main():
    """Recursively process all .md files in the target directory."""
    target_root = get_target_root()
    print(f"Processing Markdown files in: {target_root}")
