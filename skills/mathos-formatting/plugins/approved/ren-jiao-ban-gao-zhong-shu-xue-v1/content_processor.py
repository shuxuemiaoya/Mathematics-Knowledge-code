...
def apply_blank_line_fixes(text: str) -> str:
    ...
    # 论证过程空行修正：删除论证词汇前的多余空行
    text = re.sub(
        r'(?m)(?:\r?\n[ \t]*)+(?=(?:'
        # ── 长词组（4字及以上，放前面） ──
        r'解：|列方程|综上所述|根据题意|依题意|由题意|由已知|由条件|据题意|据已知|'
        r'由此可知|由此可得|这就是说|也就是说|换句话说|换言之|'
        r'同理可得|分类讨论|系数化为|整理得|化简得|配方得|经检验|经验证|经计算|经分析|经讨论|经说明|经判断|经观察|经比较|'
        r'等式两边|方程两边|等号两边|两边同乘|两边同除|两边平方|'
        r'去括号|因式分解|提取公因式|分母有理化|不失一般性|'
        # ── 中词组（2-4字） ──
        r'所以|因为|因此|因而|故而|于是|从而|进而|故此|故知|∵|∴|'
