...
2. 非TOC标题降级
   ...
   - 避免层级冲突: 将所有不在TOC中的标题强制降级到H4、H5或H6，确保不占用TOC专属的H1-H3空间
   - 保持文档结构: 降级的目标层级由你根据上下文判断，H4用于章内主要非目录标题，H5/H6用于更深的从属标题
...
1. 基本原则：
   ...
   - 完全对齐目录: 所有正文标题必须按TOC决定层级；凡是不在TOC中的标题，不能保留为H1、H2或H3
...
2. 行为准则：
   ...
   - 安全降级: 非TOC标题降级到H4-H6，具体层级由你根据目录层级、邻近章/节上下文和原始标题语义判断
   - 上下文感知: 考虑不同文档可能使用不同的标题格式前缀，规则应有足够的通用性
   - 章节上下文补全: 对重复出现或缺少父级信息的泛用标题添加章编号上下文，避免生成多个同名标题。若泛用标题本身带章编号，使用该编号；例如 `# 复习题 (11)` 应变为 `#### 第十一章 复习题 11`，`## Review Questions 5` 应变为 `#### Chapter 5 Review Questions 5`。若泛用标题不带章编号，使用 nearest preceding H1 chapter 作为当前章上下文；例如第十一章内的 `# 小节` 应变为 `#### 第十一章 小节`，Chapter 5 内的 `# Section` 应变为 `#### Chapter 5 Section`。
   - 自检循环: 输出前必须模拟应用规则并检查：(1) TOC条目全部映射为正确H1-H3；(2) 非TOC标题没有H1-H3残留；(3) `Section`、`小节`、`Review Questions 5`、`复习题 (n)` 等无父级标题已补全父级信息；(4) 规则不会匹配代码块或公式块中的伪标题。
...
- 步骤 3: 构建非TOC标题的降级规则，使用负向前瞻确保不匹配TOC中已有的标题，并将目标层级设为H4-H6
- 步骤 4: 为缺少父级信息的泛用标题生成上下文补全规则
- 步骤 5: 执行自检循环；若发现非TOC H1-H3残留、TOC标题层级错误或父级信息缺失，修正规则后重新自检
- 步骤 6: 组装最终的JSON输出，包含规则数组和简短的人类可读注释
