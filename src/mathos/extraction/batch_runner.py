    ...
    def run(self):
        for root, dirs, files in os.walk(self.vault_dir):
            for file in files:
                ...
                # Calculate dynamic output path mapping 'Dir/...' to 'Dircandidates/...'
                try:
                    rel_path = md_path.relative_to(self.vault_dir)
                    if len(rel_path.parts) >= 1:
                        top_dir = rel_path.parts[0]
                        new_top_dir = f"{top_dir}candidates"
                        json_rel_path = Path(new_top_dir).joinpath(*rel_path.parts[1:])
                        json_path = self.vault_dir / json_rel_path
                        json_path = json_path.with_suffix(".candidates.json")
                    else:
