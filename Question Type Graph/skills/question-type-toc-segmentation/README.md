...
Replace each direct child range with one standalone vault-relative Obsidian embed (`![[path/to/note.md]]`) at that same position. Never prefix a generated embed with `-` or another list marker, and embed only direct children so nested content is not rendered twice. Preserve all unassigned front matter, secondary indexes, advertisements, and appendices in the root note. Stop when the hierarchy, anchor, or adapter needs review.
