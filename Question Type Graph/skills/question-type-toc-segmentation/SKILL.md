...
subtopic. When materials contain concrete question types (`题型一`, `题型01`), automatically map each type
entry to `{folder_name}/{folder_name}.md` to create an autonomous subfolder and hub note for each type.
For organizational trees, enable the strict `leaf-only` ownership policy so questions can be owned only by
