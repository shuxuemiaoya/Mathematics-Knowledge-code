    ...
    adapter = {
        ...
        "content": {
            ...
            "question_patterns": [
                ...
                r"^(?P<number>[1-9]\d?)[.．、]\s*(?=[（(]|已知|设|若|在|如图|某|函数|曲线|向量|点|过|关于|对于|定义|记|将|用|现|四面体|正方体|长方体|直三棱柱|正三棱柱|四棱锥|三棱锥|圆|双曲线|抛物线|椭圆|数列|随机|从|取|【|\$)",
                r"^(?P<number>[（(][1-9]\d?[）)])\s*"
            ],
            ...
            "worked_example_solution_patterns": [
                r"^\s*(?:#{1,6}\s*)?【?(?:答案|解析|分析|详解|思路导航|详细解答|解答|解法|证法|证明|点拨|名师点睛|点睛|考点|总结|规律总结|试题解析|解)】?[：:]?",
                r"^\s*【答案】",
                r"^\s*【解析】",
                r"^\s*【分析】",
                r"^\s*【详解】",
