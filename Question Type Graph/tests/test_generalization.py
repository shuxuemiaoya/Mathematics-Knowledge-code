...
def test_mineru_payload_is_forced_and_format_neutral(tmp_path: Path) -> None:
...
def test_numbering_patterns_are_adapter_controlled(pattern: str, line: str, number: str) -> None:
    import re

    assert re.match(pattern, line).group("number") == number


def test_no_toc_hierarchy_accepts_reviewed_start_lines(tmp_path: Path) -> None:
    ...
    assert manifest["entries"][0]["start_line"] == 2


def test_reusable_code_has_no_sample_book_constants() -> None:
