...
SOURCE_BASE = Path("/Volumes/Whw/数学妙呀资料/高中/总复习/专题/三角函数")
VAULT_ROOT = Path("/Users/oven/Documents/ovenmathmap")
GRAPH_BASE = VAULT_ROOT / "高中" / "总复习" / "专题" / "三角函数"
...
def build_teacher_adapter(staging_path: Path, clean_title: str):
    ...
    topic_stem = clean_title.replace("-三角函数", "").strip()
...
def process_single_topic(clean_title: str, pdf_path: Path) -> bool:
    print(f"\n=======================================================")
    print(f" PROCESSING TOPIC: {clean_title}")
    print(f" PDF Path: {pdf_path.name}")
    print(f"=======================================================\n")

    staging_path = VAULT_ROOT / ".temp" / f"三角函数-{clean_title}-staging"
...
def collect_tasks():
    tasks = []
    for d in sorted(SOURCE_BASE.iterdir()):
        if not d.is_dir() or d.name.startswith("重要"):
            continue
        teacher_pdfs = sorted(d.glob("*教师版*.pdf"))
        if not teacher_pdfs:
            continue
        if len(teacher_pdfs) == 1:
            tasks.append((d.name, teacher_pdfs[0]))
        else:
            for pdf in teacher_pdfs:
                m = re.search(r"(\([0-9a-zA-Z\u4e00-\u9fa5]+\))", pdf.name)
                modifier = m.group(1) if m else f"({pdf.stem})"
                base_name = d.name.replace("-三角函数", "").strip()
                clean_name = f"{base_name}{modifier}-三角函数"
                tasks.append((clean_name, pdf))
    return tasks
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        tasks = [t for t in tasks if target_name in t[0]]

    print(f"Found {len(tasks)} tasks to process.")
    results = {}
    for idx, (clean_title, pdf_path) in enumerate(tasks, 1):
        print(f"\n[{idx}/{len(tasks)}] Processing {clean_title}...")
        passed = process_single_topic(clean_title, pdf_path)
        results[clean_title] = passed
