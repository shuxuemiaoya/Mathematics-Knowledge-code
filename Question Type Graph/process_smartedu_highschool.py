...
SOURCE_BASE = Path("/Users/oven/Downloads/中小学智慧平台资源/高中/人教版")
VAULT_ROOT = Path("/Users/oven/Documents/ovenmathmap")
MASTER_GRAPH_BASE = VAULT_ROOT / "高中" / "课堂同步" / "智慧中小学"
...
BOOK_CONFIG = {
    ...
    "选择性必修一": {
        "slug": "xuanxiu1",
        "target_name": "选择性必修第一册",
    },
    "选择性必修二": {
        "slug": "xuanxiu2",
        "target_name": "选择性必修第二册",
    },
    "选择性必修三": {
        "slug": "xuanxiu3",
        "target_name": "选择性必修第三册",
    },
}
...
def clean_lesson_title(filename: str) -> str:
    t = re.sub(r"[（(]答案解析[）)]\.pdf$", "", filename).strip()
    return t
def build_smartedu_adapter(staging_path: Path, profile_path: Path, clean_title: str, parent_rel: Path | None = None):
    ...
    if parent_rel:
        knowledge_point_name = re.sub(r"^[\d.* ]+", "", parent_rel.name).strip() or clean_title
...
def process_single_pdf(task_meta: dict, overwrite: bool = False) -> bool:
    ...
    root_md_path = graph_root / root_output_name
        ...
        if cov_p.is_file() and content_manifest.is_file() and match_manifest.is_file():
            try:
                ...
                if audit_res.get("status") == "passed":
                    print(f"[{clean_title}] ALREADY DONE & AUDIT PASSED -> Skipping.")
                    return True
            except Exception:
                pass
...
def collect_tasks_for_book(book_dir_name: str) -> list[dict]:
    if book_dir_name not in BOOK_CONFIG:
        raise ValueError(f"Unknown book: {book_dir_name}. Available: {list(BOOK_CONFIG.keys())}")

    cfg = BOOK_CONFIG[book_dir_name]
    source_root = SOURCE_BASE / book_dir_name
    master_graph_root = MASTER_GRAPH_BASE / cfg["target_name"]
    book_slug = cfg["slug"]

    pdfs = sorted(source_root.rglob("*答案解析*.pdf"))
    tasks = []
    ...
    for parent_rel, file_list in sorted(by_parent.items()):
        ...
        for pdf in file_list:
            ...
            parent_clean = re.sub(r"^[\d.* ]+", "", parent_rel.name).strip()
            ...
            staging_name = safe_name(f"smartedu_{book_slug}_{parent_rel}_{clean_title}".replace("/", "_").replace("\\", "_"))
    ...
    return tasks
def main():
    parser = argparse.ArgumentParser(description="Batch process SmartEdu High School exercise PDFs")
    parser.add_argument("--books", nargs="+", choices=list(BOOK_CONFIG.keys()), default=list(BOOK_CONFIG.keys()),
                        help="Books to process")
    parser.add_argument("--filter", help="Optional substring filter on file path")
    ...
    all_tasks = []
    for b in args.books:
        b_tasks = collect_tasks_for_book(b)
        print(f"Book [{b}] ({BOOK_CONFIG[b]['target_name']}): found {len(b_tasks)} tasks.")
        all_tasks.extend(b_tasks)

    if args.filter:
        all_tasks = [t for t in all_tasks if args.filter in str(t["pdf_path"])]
        print(f"Filter '{args.filter}' matched {len(all_tasks)} tasks.")

    if args.limit:
        all_tasks = all_tasks[:args.limit]
        print(f"Applying limit: {len(all_tasks)} tasks.")

    print(f"\n>>> Total tasks to execute: {len(all_tasks)} (Workers: {args.workers}) <<<\n")

    results = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        future_to_task = {executor.submit(process_single_pdf, t, args.overwrite): t for t in all_tasks}
        for future in concurrent.futures.as_completed(future_to_task):
            t = future_to_task[future]
            task_key = f"{t['book']} / {t['parent_rel']} / {t['clean_title']}"
            try:
