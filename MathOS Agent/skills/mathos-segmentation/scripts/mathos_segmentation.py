"""Deterministic Markdown segmentation for MathOS."""
...
SCRIPT_COMMAND = r".\skills\mathos-segmentation\scripts\mathos_segmentation.py"
...
class SegmentationError(Exception):
    """Raised when segmentation cannot continue safely."""
...
def select_target_depth(headings: list[Heading], requested_depth: int | None) -> int:
    ...
    if requested_depth is None:
        if any(heading.markdown_depth == 3 for heading in headings):
            return 3
        return max(heading.markdown_depth for heading in headings)

    if not any(heading.markdown_depth == requested_depth for heading in headings):
...
def run_record_dir(source_path: Path, repo_root: Path = Path(".")) -> Path:
    ...
    return repo_root / "agent-memory" / "records" / f"{stamp}-segmentation-{slug}"
