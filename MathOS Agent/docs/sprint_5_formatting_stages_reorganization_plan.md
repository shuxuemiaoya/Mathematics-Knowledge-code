# Sprint 5 Formatting Stages Reorganization Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Refactor the monolithic `mathos_formatting_core.py` (~1700 lines) by extracting logic into modular, stage-specific scripts while preserving 100% backward compatibility via a re-export facade.

**Architecture:** Split functionality into `mathos_common.py`, `stage1_heading.py`, `stage2_3_toc.py`, `stage4_content.py`, `stage5_optimize.py`, `program_manager.py`, and `learning_pipeline.py`. Re-export all functions from `mathos_formatting_core.py` using wildcard imports.

**Tech Stack:** Python 3.13, Pytest.

---

### Task 1: Create mathos_common.py

**Files:**
- Create: [mathos_common.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/mathos_common.py)
...
- [ ] **Step 1: Extract data classes, common exceptions, safety validators, basic string helpers, and structure parsers**

Create `mathos_common.py` containing the following contents:
```python
...
def run_plugin(plugin: ModuleType, markdown: str) -> PluginResult:
    from mathos_formatting_core import PluginResult
...
- [ ] **Step 2: Modify `mathos_formatting_core.py` to import and re-export `mathos_common` elements**

Replace the extracted code in `mathos_formatting_core.py` with imports from `mathos_common`:
```python
from mathos_common import (
    Heading, TextBlock, H1Section, MarkdownStructure, PreservationCounts,
    ApprovedApplyResult, CandidateRunResult, LearningRunState, LearningRunResult,
    FormattingError, SAFE_IMPORTS, UNSAFE_CALL_NAMES, UNSAFE_ATTRIBUTE_ROOTS,
    load_safe_plugin, run_plugin, candidate_path_for, learning_work_dir_for,
    learning_candidate_path_for, write_learning_state, create_fresh_candidate,
    HEADING_RE, ATX_CLOSING_SEQUENCE_RE, TOC_HEADING_RE, HEADING_LIKE_RE,
    TOC_ENTRY_PAGE_RE, FLAG_MAP, CHINESE_CHAPTER_RE, ENGLISH_CHAPTER_RE,
    _is_code_fence_close, _match_code_fence_open, _normalize_toc_page_heading,
    _normalize_atx_heading_text, _line_offsets, _chinese_number_to_int,
    _int_to_chinese_number, _normalize_chinese_chapter_number,
    _chapter_context_from_heading_text, _extract_protected_blocks,
    _line_in_blocks, _extract_toc_block, _extract_h1_sections,
    extract_structure, _sha256_text, _write_text_artifact,
    _strip_single_line_ending, _split_single_line_ending
)
```

- [ ] **Step 3: Run pytest to verify everything passes**
...
```bash
git add skills/mathos-formatting/scripts/mathos_common.py skills/mathos-formatting/scripts/mathos_formatting_core.py
git commit -m "refactor: extract common utils and models to mathos_common.py"
```

---

### Task 2: Create stage1_heading.py

**Files:**
- Create: [stage1_heading.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/stage1_heading.py)
- Modify: [mathos_formatting_core.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/mathos_formatting_core.py)

- [ ] **Step 1: Extract Stage 1 heading refinement and audit logic**

Create `stage1_heading.py` containing the following contents:
```python
from __future__ import annotations
import re
from dataclasses import dataclass
...
- [ ] **Step 2: Add HeadingRule export to `mathos_common.py`**

Open `mathos_common.py` and make sure `HeadingRule` is defined there:
```python
@dataclass(frozen=True)
class HeadingRule:
    rule_id: str
...
- [ ] **Step 3: Modify `mathos_formatting_core.py` to import and re-export `stage1_heading` elements**

In `mathos_formatting_core.py`, replace heading functions with imports:
```python
from stage1_heading import (
    apply_heading_rules, validate_heading_rules, audit_stage1_headings
)
```

- [ ] **Step 4: Run pytest to verify everything passes**
...
```bash
git add skills/mathos-formatting/scripts/stage1_heading.py skills/mathos-formatting/scripts/mathos_formatting_core.py
git commit -m "refactor: extract stage 1 heading logic to stage1_heading.py"
```

---

### Task 3: Create stage2_3_toc.py

**Files:**
- Create: [stage2_3_toc.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/stage2_3_toc.py)
- Modify: [mathos_formatting_core.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/mathos_formatting_core.py)

- [ ] **Step 1: Extract TOC sampling, detection, stripping, and H1 sample extraction**

Create `stage2_3_toc.py` containing the following contents:
```python
from __future__ import annotations
from pathlib import Path
from mathos_common import (
...
- [ ] **Step 2: Add page extraction helpers to `mathos_common.py`**

Ensure that `find_total_pages_from_metadata` and `extract_first_20_pages` are defined in `mathos_common.py` (they were extracted there in Task 1).

- [ ] **Step 3: Modify `mathos_formatting_core.py` to import and re-export `stage2_3_toc` elements**

In `mathos_formatting_core.py`, replace TOC functions with imports:
```python
from stage2_3_toc import (
    extract_toc_sample, extract_h1_sample
)
```
...
```bash
git add skills/mathos-formatting/scripts/stage2_3_toc.py skills/mathos-formatting/scripts/mathos_formatting_core.py
git commit -m "refactor: extract stage 2/3 TOC helpers to stage2_3_toc.py"
```

---

### Task 4: Create stage4_content.py

**Files:**
- Create: [stage4_content.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/stage4_content.py)
- Modify: [mathos_formatting_core.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/mathos_formatting_core.py)

- [ ] **Step 1: Extract Stage 4 content cleanup and validation logic**

Create `stage4_content.py` containing the following contents:
```python
from __future__ import annotations
import re
from dataclasses import dataclass
...
- [ ] **Step 2: Add shared content helper definitions to `mathos_common.py`**

In `mathos_common.py`, define the following helpers (copied from the original `core.py`):
```python
...
- [ ] **Step 3: Modify `mathos_formatting_core.py` to import and re-export `stage4_content` elements**

In `mathos_formatting_core.py`, replace content cleaner functions with imports:
```python
from stage4_content import (
    ContentRule, validate_content_rules, run_content_rules,
    content_preservation_counts, preservation_summary,
    validate_content_preservation, run_content_plugin_protecting_headings,
...
```bash
git add skills/mathos-formatting/scripts/stage4_content.py skills/mathos-formatting/scripts/mathos_common.py skills/mathos-formatting/scripts/mathos_formatting_core.py
git commit -m "refactor: extract stage 4 content logic to stage4_content.py"
```

---

### Task 5: Create stage5_optimize.py

**Files:**
- Create: [stage5_optimize.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/stage5_optimize.py)
- Modify: [mathos_formatting_core.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/mathos_formatting_core.py)

- [ ] **Step 1: Extract Stage 5 heading optimization, diffing, and reporting logic**

Create `stage5_optimize.py` containing the following contents:
```python
from __future__ import annotations
import difflib
import json
...
- [ ] **Step 2: Add JSON parsing helper to `mathos_common.py`**

Ensure `parse_json_artifact_from_text` is exported from `mathos_common.py`.

- [ ] **Step 3: Modify `mathos_formatting_core.py` to import and re-export `stage5_optimize` elements**

In `mathos_formatting_core.py`, replace optimize/report functions with imports:
```python
from stage5_optimize import (
    unified_markdown_diff, write_review_report, run_heading_optimization
)
```
...
```bash
git add skills/mathos-formatting/scripts/stage5_optimize.py skills/mathos-formatting/scripts/mathos_formatting_core.py
git commit -m "refactor: extract stage 5 optimize logic to stage5_optimize.py"
```

---

### Task 6: Create program_manager.py and learning_pipeline.py

**Files:**
- Create: [program_manager.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/program_manager.py)
- Create: [learning_pipeline.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/learning_pipeline.py)
- Modify: [mathos_formatting_core.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/mathos_formatting_core.py)

- [ ] **Step 1: Extract program manager (saving, applying, run candidate)**

Create `program_manager.py` containing the following contents:
```python
from __future__ import annotations
import json
import re
...
from mathos_common import (
    ...
    create_fresh_candidate, run_content_rules_protecting_headings,
    run_content_plugin_protecting_headings, ApprovedApplyResult, CandidateRunResult
)
from stage1_heading import validate_heading_rules, apply_heading_rules
from stage4_content import validate_content_rules
...
- [ ] **Step 2: Extract learning pipeline coordinator**

Create `learning_pipeline.py` containing the following contents:
```python
from __future__ import annotations
import json
import shutil
...
- [ ] **Step 3: Modify `mathos_formatting_core.py` to import and re-export `program_manager` and `learning_pipeline` elements**

In `mathos_formatting_core.py`, replace program and pipeline functions with imports:
```python
from program_manager import (
    save_approved_program, apply_approved_program, run_candidate_from_artifacts
)
from learning_pipeline import (
    run_learning_from_provider
)
```
...
```bash
git add skills/mathos-formatting/scripts/program_manager.py skills/mathos-formatting/scripts/learning_pipeline.py skills/mathos-formatting/scripts/mathos_formatting_core.py
git commit -m "refactor: extract program_manager and learning_pipeline modules"
```

---

### Task 7: Finalize mathos_formatting_core.py Re-export Facade

**Files:**
- Modify: [mathos_formatting_core.py](file:///C:/Mathematics-Knowledge/Mathematics-Knowledge-code/skills/mathos-formatting/scripts/mathos_formatting_core.py)

- [ ] **Step 1: Clean up mathos_formatting_core.py to contain only clean imports and wildcard re-exports**

Rewrite `mathos_formatting_core.py` to be a pure facade:
```python
...
- [ ] **Step 2: Run pytest to ensure everything works seamlessly**
...
git commit -m "refactor: turn mathos_formatting_core.py into a pure facade re-exporting all stage modules"
```
